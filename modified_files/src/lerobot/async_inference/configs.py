...
class PolicyServerConfig:
    ...
    # Real-Time Chunking (RTC) overrides. RTC guidance is otherwise driven entirely by the
    # checkpoint's saved rtc_config; these let you force it on (or retune it) without editing the
    # checkpoint. Only affect flow-matching policies (pi0 / pi05 / smolvla).
    enable_rtc: bool = field(
        default=False,
        metadata={
            "help": "Force-enable RTC guidance even if the checkpoint's rtc_config is null or "
            "disabled. No effect on policies that do not support RTC (e.g. ACT)."
        },
    )
    rtc_execution_horizon: int | None = field(
        default=None,
        metadata={
            "help": "Override RTC execution_horizon. If None, uses the checkpoint's rtc_config "
            "value, or the RTCConfig default when force-enabling."
        },
    )
    rtc_reset_gap_s: float = field(
        default=0.5,
        metadata={
            "help": "Flush the RTC chunk cache when an observation arrives more than this many "
            "seconds after the last cached chunk (e.g. after homing / a pause-resume). The first "
    ...
    def __post_init__(self):
        ...
        if self.rtc_execution_horizon is not None and self.rtc_execution_horizon <= 0:
            raise ValueError(
